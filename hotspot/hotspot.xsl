...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns="http://www.w3.org/1999/xhtml" xmlns:html="http://www.w3.org/1999/xhtml" xmlns:hotspot="http://dret.net/xmlns/hotspot/1" xmlns:kilauea="http://xmlns.sharpeleven.net/kilauea" xpath-default-namespace="http://dret.net/xmlns/hotspot/1" exclude-result-prefixes="xs hotspot html kilauea">
	...
	<xsl:variable name="default-configuration" as="element(hotspot:configuration)" xmlns="http://dret.net/xmlns/hotspot/1">
		<configuration>
			...
			<listing class="listing" default-title="" alternate-links-separator="; "/>
			<outline title="Outline" hidden-title="yes" count-text=" (* Slides)" count-depth="2"/>
			<hyperlink intra="" inter="" extra="→ *" print-href=" [*]" media-info=" (* version)"/>
			...
			<misc title-separator=" ; " generate-IDs="no" figure-number="Fig. *: "/>
	...
	<xsl:template match="/">
		...
		<xsl:apply-templates select="$preprocessed/hotspot">
			...
			<xsl:with-param name="categories" select="hotspot/categories/category" tunnel="yes"/>
		</xsl:apply-templates>
		<!-- say goodbye! -->
		<xsl:call-template name="message">
	...
	<!-- . . . . . . . . . . . . . . . . . . . . . . . . . . .-->
	<!--. . . . . . . . . . . . . . . . . . . . . . . . . . . -->
	<xsl:template match="presentation//*[local-name() = $index-elements and empty(@href)]" mode="preprocess">
		<xsl:copy>
	...
	<!-- prepare figures                                      -->
	<!-- .................................................... -->
	<xsl:template match="img | html:img" mode="preprocess">
		<xsl:choose>
			<xsl:when test="exists(caption)">
				<div class="figure">
					<!-- todo: handle floats and dimensions from the img -->
					<xsl:next-match/>
					<div class="caption">
						<span class="fignum">
	...
	<!-- . . . . . . . . . . . . . . . . . . . . . . . . . . .-->
	<!-- '''''''''''''''''''''''''''''''''''''''''''''''''''' -->
	<!-- prepare the @href-bearing elements                   -->
	<!-- .................................................... -->
	<xsl:template match="*[@href][not(local-name() = ('a', 'area', 'base', 'link', 'listing', 'img'))]" mode="preprocess">
		<a href="{@href}">
			<xsl:copy>
				<xsl:if test="local-name() = $index-elements and not(@index)">
					<xsl:attribute name="index" select="hotspot:text(.)"/>
				</xsl:if>
...
<!--				<span class="{key('categoryKey', local-name())/@class}" id="{generate-id(.)}">-->
	...
	<xsl:template match="presentation">
		...
		<!-- 2018/04/22; dret: "simple-indent" was "simple{ if ($params/@indent eq 'yes') then '-indent' else '' }", but that did not seem to work. -->
		<xsl:result-document format="slides-indent" href="{hotspot:presentationfilename(., $configuration)}">
<!--			<xsl:text>&#xA;</xsl:text>-->
			<xsl:comment>
				<xsl:text> Do not edit by hand! Generated by Hotspot on </xsl:text>
...
<!--						<script type="text/javascript" src="{$kilauea-dir}/js/kilauea.packed.js"/>-->
	...
	<xsl:template name="presentation" as="element()*">
		...
		<xsl:apply-templates select="$layout/class | $layout/background"/>
		...
		<!-- process the layout's presentation template -->
		<xsl:apply-templates select="$layout/presentation/*">
			<xsl:with-param name="presentation" select="." tunnel="yes"></xsl:with-param>
		</xsl:apply-templates>
	</xsl:template>
	<!-- . . . . . . . . . . . . . . . . . . . . . . . . . . .-->
	<!--. . . . . . . . . . . . . . . . . . . . . . . . . . . -->
	<xsl:template match="layout/presentation//cover">
		<xsl:param name="layout" as="element(hotspot:layout)" tunnel="yes"/>
		<xsl:apply-templates select="$layout/cover"/>
	</xsl:template>
	<!-- . . . . . . . . . . . . . . . . . . . . . . . . . . .-->
	<!--. . . . . . . . . . . . . . . . . . . . . . . . . . . -->
	<xsl:template match="layout/presentation//static-toc">
		<xsl:param name="presentation" tunnel="yes"/>
		<ul>
			<xsl:apply-templates select="$presentation/slide | $presentation/part" mode="static-toc"/>
		</ul>
	</xsl:template>
	<!-- . . . . . . . . . . . . . . . . . . . . . . . . . . .-->
	<!--. . . . . . . . . . . . . . . . . . . . . . . . . . . -->
	<xsl:template match="layout/presentation//contents">
		<xsl:param name="presentation" tunnel="yes"/>
		<xsl:apply-templates select="$presentation/slide | $presentation/outline | $presentation/part"/>
	</xsl:template>
	<!-- . . . . . . . . . . . . . . . . . . . . . . . . . . .-->
	<!--. . . . . . . . . . . . . . . . . . . . . . . . . . . -->
	<xsl:template match="slide" mode="static-toc">
	...
	<xsl:template match="part" mode="static-toc">
		<xsl:if test="exists(slide | part)">
			<li class="part-entry">
				...
				<xsl:apply-templates select="title/node()"/>
				<ul>
					<xsl:apply-templates select="slide | part" mode="static-toc"/>
				</ul>
			</li>
		</xsl:if>
	</xsl:template>
	...
	<!-- background slides                                    -->
	<!-- .................................................... -->
	<xsl:template match="class | background">
	...
	<xsl:template match="slide">
		...
		<div class="slide{ if ( exists(@class) ) then concat(' ', normalize-space(@class)) else ''}">
			<!-- only use an ID if desired or given -->
			<xsl:if test="exists(@id) or $configuration/misc/@generate-IDs = 'yes'">
				<xsl:attribute name="id" select="if (exists(@id)) then @id else @generated-id"/>
			</xsl:if>
			<!-- process the layout's slide template -->
			<!-- todo: make multiple slide templates selectable through @class -->
			<xsl:apply-templates select="$layout/slide/*">
				<xsl:with-param name="slide" select="." tunnel="yes"/>
				<xsl:with-param name="shortcut-stack" as="element(hotspot:shortcuts)+" tunnel="yes">
					<xsl:call-template name="push-shortcuts"/>
				</xsl:with-param>
			</xsl:apply-templates>
	...
	<!-- . . . . . . . . . . . . . . . . . . . . . . . . . . .-->
	<!--. . . . . . . . . . . . . . . . . . . . . . . . . . . -->
	<xsl:template match="layout/slide//contents">
		<xsl:param name="slide" tunnel="yes"></xsl:param>
		<xsl:apply-templates select="$slide/*[not(local-name() = $shortcuts and node())]"></xsl:apply-templates>
	</xsl:template>
	<!-- . . . . . . . . . . . . . . . . . . . . . . . . . . .-->
	<!--. . . . . . . . . . . . . . . . . . . . . . . . . . . -->
	<xsl:template match="layout/slide//h1[.//title]">
		<xsl:param name="shortcut-stack" as="element(hotspot:shortcuts)+" tunnel="yes"/>
		<h1>
			<xsl:if test="exists($shortcut-stack[last()]/title/@short)">
				<xsl:attribute name="title">
					<xsl:value-of select="$shortcut-stack[last()]/title/@short"/>
				</xsl:attribute>
			</xsl:if>
			<xsl:apply-templates/>
		</h1>
...
<!--				<xsl:variable name="title" select="concat(hotspot:expand-shortcut(hotspot:push-shortcuts($shortcut-stack, parent::part), 'title', 'short'), ':', hotspot:expand-shortcut(hotspot:push-shortcuts($shortcut-stack, .), 'title', 'short'))"/>-->
	...
	<!-- . . . . . . . . . . . . . . . . . . . . . . . . . . .-->
	<!--. . . . . . . . . . . . . . . . . . . . . . . . . . . -->
	<xsl:template match="slidenum">
		<xsl:param name="slide" tunnel="yes"></xsl:param>
		<xsl:choose>
			<xsl:when test="ancestor::presentation">
				<xsl:value-of select="hotspot:slidenumber(ancestor::slide)"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:value-of select="hotspot:slidenumber($slide)"/>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	<!-- . . . . . . . . . . . . . . . . . . . . . . . . . . .-->
	...
	<xsl:template match="img">
		<xsl:param name="configuration" as="element(hotspot:configuration)" tunnel="yes"/>
		<xsl:variable name="img" as="element(html:img)">
			<img>
		...
		<pre class="{$configuration/listing/@class}{ if ( exists(@class) ) then concat(' ', @class) else '' }">
			<xsl:choose>
				...
				<xsl:when test="exists(@href)">
					...
					<a title="{ if ( exists(@title) ) then @title else @href }">
						<xsl:apply-templates select="@href"/>
						<xsl:sequence select="$listing"/>
					</a>
				</xsl:when>
	...
	<xsl:template match="link">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				...
				<xsl:if test="$configuration/hyperlink/@print-href">
					<span class="print-href">
						<xsl:value-of select="substring-before($configuration/hyperlink/@print-href, '*')"/>
<!--						<xsl:value-of select="concat(if ( string-length($presentation-name) eq 0 ) then '' else hotspot:title(key('structureIdKey', @href)/ancestor-or-self::presentation, 'long', 'string'), if ( exists(key('structureIdKey', @href)/self::presentation) ) then '' else concat(if ( string-length($presentation-name) eq 0 ) then ''  else '; ', hotspot:title(key('structureIdKey', @href),'long', 'string'), ' (', hotspot:slidenumber(key('structureIdKey', @href)/ancestor::slide), ')'))"/>-->
						<xsl:choose>
							<xsl:when test="string-length($presentation-name) eq 0"></xsl:when>
							<xsl:otherwise>
								<xsl:apply-templates select="hotspot:title(key('structureIdKey', @href)/ancestor-or-self::presentation, 'long', 'nodes')/node()"></xsl:apply-templates>
							</xsl:otherwise>
						</xsl:choose>
						<xsl:choose>
							<xsl:when test="exists(key('structureIdKey', @href)/self::presentation)"></xsl:when>
							<xsl:otherwise>
								<xsl:choose>
									<xsl:when test="string-length($presentation-name) eq 0"></xsl:when>
									<xsl:otherwise>
								...
								<xsl:apply-templates select="hotspot:title(key('structureIdKey', @href), 'long', 'nodes')/node()"/>
								<xsl:value-of select="concat(' (', hotspot:slidenumber(key('structureIdKey', @href)/ancestor::slide), ')')"/>
							</xsl:otherwise>
						</xsl:choose>
<!--						<xsl:value-of select="concat(if ( string-length($presentation-name) eq 0 ) -->
<!--						then '' -->
<!--						else hotspot:title(key('structureIdKey', @href)/ancestor-or-self::presentation, 'long', 'nodes')/node(), -->
<!--						if ( exists(key('structureIdKey', @href)/self::presentation) ) -->
<!--						then '' -->
<!--						else concat(-->
<!--							if ( string-length($presentation-name) eq 0 ) -->
<!--							then ''  -->
<!--							else '; ', -->
<!--						hotspot:title(key('structureIdKey', @href),'long', 'nodes')/node(), ' (', hotspot:slidenumber(key('structureIdKey', @href)/ancestor::slide), ')'))"/>-->
						<xsl:value-of select="substring-after($configuration/hyperlink/@print-href, '*')"/>
					</span>
				</xsl:if>
			</xsl:otherwise>
		</xsl:choose>
	...
	<xsl:template match="a | html:a">
		...
		<xsl:element name="a" namespace="http://www.w3.org/1999/xhtml">
			...
			<xsl:apply-templates select="@*[not(local-name() = ('class', 'href'))]"/>
		...
		<!-- if the link bears @rel="alternate" and @media, print out @media, formatted through $configuration/hyperlink/@media-info -->
		<xsl:if test="$configuration/hyperlink/@media-info and @rel='alternate' and exists(@media)">
			<span class="media-info">
				<xsl:value-of select="substring-before($configuration/hyperlink/@media-info, '*')"/>
				<xsl:value-of select="@media"/>
				<xsl:value-of select="substring-after($configuration/hyperlink/@media-info, '*')"/>
			</span>
		</xsl:if>
		<!-- if $configuration/hyperlink/@print-href is present, use this in order to print out a formatted text version of the href -->
		<xsl:if test="$configuration/hyperlink/@print-href">
			<span class="print-href">
				<xsl:value-of select="substring-before($configuration/hyperlink/@print-href, '*')"/>
...
<!--	<xsl:template match="*[@href][not(local-name() = ('a', 'area', 'base', 'link', 'listing', 'img'))]">-->
<!--		<xsl:param name="configuration" as="element(hotspot:configuration)" tunnel="yes"/>-->
<!--		<a href="{@href}">-->
...
<!--	</xsl:template>-->
	...
	<xsl:template match="@src | @data | @document" mode="layout">
		...
		<xsl:attribute name="{local-name()}" select="concat($layout-dir, $current-layout, '/', .)"/>
	</xsl:template>
	<!-- . . . . . . . . . . . . . . . . . . . . . . . . . . .-->
	<!--. . . . . . . . . . . . . . . . . . . . . . . . . . . -->
	<!-- . . . . . . . . . . . . . . . . . . . . . . . . . . .-->
